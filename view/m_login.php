<?php
	...
	$user = null;
	$url = $_POST['url'];
	if(strlen(trim($account))>0 && strlen(trim($password))>0){
				...
				<form action="/login" method="post">
					...
					<div class="form-row">
						<div class="input-frame <?php if($user['user_id']===0){echo ' selected warn ';}?>">
							...
							<em></em>
							<input type="text" name="account" id="account" placeholder="3-20位字符" value="<?php echo $account;?>">
						</div>
					</div>
					<div class="form-row">
